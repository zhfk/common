...
double getCurrentTimestamp() {       // Linux
  timespec a;
			 ...
			 for(size_t  i = 0; i < numDevices; ++i)
			  {
				...
				//获取设备频率
				clGetDeviceInfo(devices[i], CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(cl_ulong), &long_entries, NULL);
				printf("%-40s = %d\n","CL_DEVICE_MAX_CLOCK_FREQUENCY(MHz)",long_entries);

				//数据并行执行模块用来声明global和localwork-item IDS的最大维度。该参数最小为3
				clGetDeviceInfo(devices[i], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS, sizeof(cl_ulong), &long_entries, NULL);
				printf("%-40s = %d\n","CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS",long_entries);

				//获取最大工作组数
				clGetDeviceInfo(devices[i], CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(cl_ulong), &long_entries, NULL);
				printf("%-40s = %d\n","CL_DEVICE_MAX_WORK_GROUP_SIZE",long_entries);

				//获取每个维度work-item size   数组类型
				clGetDeviceInfo(devices[i], CL_DEVICE_MAX_WORK_ITEM_SIZES,3* sizeof(cl_ulong),MaxItemSize, NULL);
				printf("%-40s = <%d,%d,%d>\n","CL_DEVICE_MAX_WORK_ITEM_SIZES",MaxItemSize[0],MaxItemSize[1],MaxItemSize[2]);

				//获取最大计算核心数
				clGetDeviceInfo(devices[i], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(cl_ulong), &long_entries, NULL);
				...
				//最大采样器数目
				clGetDeviceInfo(devices[i], CL_DEVICE_MAX_SAMPLERS, sizeof(cl_ulong), &long_entries, NULL);
